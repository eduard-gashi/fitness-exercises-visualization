...
export default function DumbbellChart({ data, colors }) {
    ...
    useEffect(() => {
        ...
        metrics.forEach(metric => {
            ...
            // Fix invisible line if values are equal
            if (x1 === x2) {
                x1 -= 1;
                x2 += 1;
            }
        ...
        // X axis
        chart.append("g")
            .attr("transform", `translate(0, ${innerHeight + 0})`)
            .call(d3.axisBottom(x).ticks(10))
            .selectAll("text")
            .style("font-size", "10px")


        const legendYStart = innerHeight + 40; // Start position below x-axis
        const legendSpacing = 20;              // Vertical space between entries
